...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	private void parseDefaultElement(Element ele, BeanDefinitionParserDelegate delegate) {
		// 解析import标签，主要用于多个配置文件组成一个applicationContext
	...
	protected void importBeanDefinitionResource(Element ele) {
		...
		if (absoluteLocation) {
			try {
				// 将解析到的数据存放在actualResources中，唯一值，并返回import数量
	 ...
	 * 对于bean，解析这个bean的定义以及将这个bean进行注册
	 */
	protected void processBeanDefinition(Element ele, BeanDefinitionParserDelegate delegate) {
		// 进行元素的解析 反正值bdHolder包含了bean的所有信息
		BeanDefinitionHolder bdHolder = delegate.parseBeanDefinitionElement(ele);
		if (bdHolder != null) {
			// 如果需要的话就对bean进行装饰
			bdHolder = delegate.decorateBeanDefinitionIfRequired(ele, bdHolder);
			try { 
				// Register the final decorated instance. 对bean进行注册
			...
			// Send registration event. 发送注册成功事件
