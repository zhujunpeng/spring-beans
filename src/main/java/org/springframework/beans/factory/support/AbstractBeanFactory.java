...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(
		 ...
		 * 为什么首先会使用这段代码呢?
		 * 因为在创建单例bean的时候会存在依赖注入的情况，而在创建的时候为了避免循环依赖
		 * spring创建bean的原则是不等bean创建完成就会将创建bean的ObjectFactory提前曝光也就是将ObjectFactory加入到缓存中
		...
		if (sharedInstance != null && args == null) {
			...
			// 返回对应的实例，有时候存在比如BeanFactory的情况并不是直接返回实例本身而是返回指定方法返回的实例
			...
			// 如果beanDefinitionMap中也就是在所有已经加载类中不包括beanName则尝试从parentBeanFactory中检测
	 ...
	 * 返回流程实例
	 * 
	 * 1、判断beanInstance的类型，如果不是FactoryBean那么直接返回
	 * 2、如果是FactoryBean那么查询缓存中是否存在这个bean，查询返回，否则就直接通过getObject获取这个bean直接返回
	 * 
	...
	protected Object getObjectForBeanInstance(
		...
		// 如果指定的name是工厂相关（以&为前缀）,同时beanInstance又不是FactoryBean类型，那么验证不通过
		...
		/**
		 * 现在有了流程实例，这个实例可能会是正常的bean或者是FactoryBean
		 * 如果是FactoryBean，那么使用这个创建实例，但是如果用户想要直接获取工厂实例而不是工厂的getObject方法
		 * 对应的实例那么传入的bean也应该加入前缀&
		 */
		if (!(beanInstance instanceof FactoryBean) || BeanFactoryUtils.isFactoryDereference(name)) {
			return beanInstance;
		}
		...
		if (object == null) {
			...
			// 前面判断了!(beanInstance instanceof FactoryBean 到这就肯定是FactoryBean类型了 一个工厂类型，
			FactoryBean<?> factory = (FactoryBean<?>) beanInstance;
			// Caches object obtained from FactoryBean if it is a singleton.
			// containsBeanDefinition检测beanDefinitionMap中，也就是所有已经加载的类中检测是否定义了beanName
