...
class ConstructorResolver {
	 ...
	 * @param mbd the merged bean definition for the bean  从xml文件总获取到的bean的所有信息
	...
	public BeanWrapper autowireConstructor(
		...
		if (explicitArgs != null) {
			...
			// 如果通过getBean方法的时候没有指定则尝试从配置文件中解析
			...
			if (argsToResolve != null) {
				/**
				 * 解析参数类型，如给定方法的构造函数A（int,int）则通过此方法后就会把配置文件中的（"1","1"）转换成（1，1）
				 * 缓存中的值可能是原始值也可能是最终值（因为通过了转换）
				 */
		...
		if (constructorToUse == null) {
			...
			// 所有的构造函数集合，一个类可能存在很多个构造函数
			Constructor<?>[] candidates = chosenCtors;
			if (candidates == null) {
				// 通过反射的方式获取当前Javabean的信息
				Class<?> beanClass = mbd.getBeanClass();
				try {
					// 返回是否允许访问非公共构造函数和方法。返回所有的构造函数
