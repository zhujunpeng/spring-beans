...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * bean的作用范围，对应bean的scop
	 */
	private String scope = SCOPE_DEFAULT;
	/**
	 * bean是否为单例，来自scope（默认为单例模式）
	 */
	...
	/**
	 * 自动注入模式，对应bean的autowire 默认是false
	 */
	...
	/**
	 * autowireCandidate设置为false时，这样容器在查找自动装配对象时将不考虑改bean，
	 * 即它不会被考虑作为其他bean自动装配的候选者，但该bean本身还是可以使用自动装配来实现注入其他bean
	 */
	private boolean autowireCandidate = true;
	/**
	 * 自动装配多个bean候选者时，作为首候选者，优先加载
	 */
	...
	/**
	 * 记录构造器注入属性，对应bean的construtor-arg属性
	 */
	...
	/**
	 * 方法重写记录者，记录lookup-method、replace-method
	 */
	...
	/**
	 * 是否为用户定义而不是应用程序本身定义的，创建AOP时为true
	 */
